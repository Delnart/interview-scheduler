...
router.get('/general', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const rows = db.prepare(`${SELECT_BOOKED} ORDER BY matched_slots.start_time`).all();
...
router.get('/recruiter/:id', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const recruiter = db.prepare('SELECT id, full_name FROM recruiters WHERE id = ?').get(req.params.id);
...
router.get('/oauth/status', requireAuth, (req, res) => {
  res.json({ connected: googleCalendar.isConnected(req.user.id), configured: googleCalendar.isConfigured() });
});
...
router.get('/oauth/callback', async (req, res) => {
  ...
  try {
    ...
    googleCalendar.saveTokens(payload.id, tokens);
...
router.post('/oauth/disconnect', requireAuth, (req, res) => {
  googleCalendar.clearTokens(req.user.id);
  res.json({ ok: true });
});
