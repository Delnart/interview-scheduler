...
router.get('/', requireAuth, requireAdmin, (req, res) => {
  const rows = db.prepare('SELECT * FROM recruiters ORDER BY full_name').all();
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM recruiters WHERE id = ?').get(req.params.id);
...
router.post('/', requireAuth, requireAdmin, (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM recruiters WHERE email = ?').get(email.toLowerCase().trim());
  ...
  const row = db.prepare('SELECT * FROM recruiters WHERE id = ?').get(id);
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  if (next.email !== row.email) {
    const existing = db.prepare('SELECT id FROM recruiters WHERE email = ? AND id != ?').get(next.email, row.id);
    if (existing) return res.status(409).json({ error: 'Користувач з таким email вже існує' });
  }
  ...
  const updated = db.prepare('SELECT * FROM recruiters WHERE id = ?').get(row.id);
...
router.post('/:id/reset-password', requireAuth, requireAdmin, (req, res) => {
  ...
  const row = db.prepare('SELECT id FROM recruiters WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Не знайдено' });

  db.prepare('UPDATE recruiters SET password_hash = ? WHERE id = ?').run(bcrypt.hashSync(parsed.data.newPassword, 10), row.id);
  res.json({ ok: true });
});
...
router.delete('/:id', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  db.prepare('UPDATE recruiters SET active = 0 WHERE id = ?').run(row.id);

  // Regenerate matched slots for any OP this recruiter is part of, since their
  // availability should no longer be considered.
  const ops = db.prepare('SELECT DISTINCT op_code FROM op_recruiters WHERE recruiter_id = ?').all(row.id);
